...
Most common: multiple Xs (predictors)  
               ...
               my note: behavior ~ attititude + perceived norms + perceived agency
...
While some can have multiple Ys (outcomes)  
               ...
               my note: behavior + intention ~ attititude + perceived norms + perceived agency
...
mynote: PCA when there's a lot of x variable and tryin to see the correlation between them.
...
Plotting  
mynote: it is important to check this to what is happening with the x variable
...
weather %>%
  ...
  ungroup() %>%
  slice(1:6) %>% 
  unnest(data) %>% #my note: instead of unnesting, this can be done using mutate for iteration
  ggplot(aes(x = value, 
             y = strength_gtex))+
  geom_point(shape = 21, 
             alpha = .7, 
...
# Multicollinearity 
## Concepts  
Multicollinearity definition: more than two explanatory variables (x) in a multiple regression model are highly linearly related.  
...
- Model estimates (slopes) standard error are inflated, directly affecting p-values, estimate significance, and power.  
...
weather %>%
  ...
  #stat_correlation()
```
my note: # is for commenting out and the above shows that they are not highly correlated.method="lm" : this draws the line as a linear regression that is straight
...
```
These are very highly correlated.
...
lm_dayl.prcp <- lm(strength_gtex ~ mean_dayl.s_Apr + 
                   sum_prcp.mm_Oct,
   data = weather) %>% #explaining strength as a linear function of mean_dayl.s_Apr andsum_prcp.mm_Oct
  tidy() %>%
  filter(term != "(Intercept)") %>%
...
```{r vif correlated}
# highly Correlated multiple x variable
...
weather_n <- weather%>%
  dplyr::select(-site, -year, -strength_gtex)

weather_n
```
my note: standardizing so they follow similar scale

# PCA  
Principal component analysis (PCA) is a dimensionality reduction approach that accomodates only numerical variables.  

my note: The goal of PCA is to decrease the number of dimensions in the model. Two reasons are to reduce the number of dimensions and to have dimensions that are uncorrelated that may suffer for multicolinearity.
watch class video on multicolinearity and PCA
...
my note: PC1 explains the most variance that is the direction you have the most variance.
...
summary(mod_pca)
```
my note: Each PC is combination of all original variables.
...
PCs 1 and 2 explain ~35% and ~26% (61%) of total variance of the original 72 weather variables.
...
My note: proportion of variance explains the proportion of variance explained in each PCs (PC1, PC2, PC3, PC4)
We would need 4 PCs.  
...
mynote: the code below and plot is for PC1 and can be repeated for PC2 - PC3
```{r PC1 weights}
mod_pca$rotation %>%
  as.data.frame() %>%
...
```
my note: PC1 is on the x axis as Dim1 (35%) while PC2 is on the y axis as Dim 2(26.4%)
  
The longer the eigenvector for a given variable, the more important it is towards that PC.  
...
weather_postpca <- weather%>%
  bind_cols(pca_scores) #right_join would have worked here if we had one similar column
